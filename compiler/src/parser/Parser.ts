...
export class Parser {
  ...
  parse(): AST.ProgramNode {
    const module = this.parseModule("unknown.easys");

    if (!module.app) {
      throw this.error("Expected 'app' declaration");
    }
    ...
    return this.expandProgram(program);
  }

  /** Parse a file without resolving imports or expanding components. */
  parseModule(filePath: string): AST.ModuleNode {
    ...
    let app: AST.AppNode | undefined;

    this.skipNewLines();

    if (!this.isEOF() && this.current().value === "app") {
      app = this.parseApp();
    }
  ...
  private parseImport(): AST.ImportNode {
    ...
    // import Header from "./path.easys"
    const name = this.expectIdentifier();
    this.skipNewLines();

    if (this.current().value !== "from") {
      throw this.error("Expected 'from' after import name");
    }
